...
```json
{
  ...
  "layout": {
    ...
    "links": [
      { "id": "l1", "from": "client-01", "to": "lb-01", "protocol": "sync", "traffic": 80 },
  ...
  "scenarios": [
    {
      ...
      "actions": [
        { "targetId": "l1", "props": { "traffic": { "forward": 100, "backward": 20 }, "color": "#ff6600" } },
...
| `layoutSettings` | `Object` | Optional | Algorithmic parameters for deterministic spacing/flow. |
| `layoutSeed` | `String\|Number` | Optional | A salt/seed for the PRNG to guarantee deterministic auto-layout calculation. |
...
Mathematical parameters fed to the auto-layout calculation function (Dagre) to ensure 100% determinism.
| Key | Type | Requirement | Description |
| :--- | :--- | :--- | :--- |
| `ranksep` | `Number` | Optional | Distance between layers (e.g., Client -> LB). Defaults to `100`. |
| `nodesep` | `Number` | Optional | Distance between sibling nodes in the same layer. Defaults to `50`. |
| `edgesep` | `Number` | Optional | Distance between parallel edge paths. Defaults to `20`. |
...
| `traffic` | `Number\|Object` | Optional | Density of flow animation. Can be a `Number` (0-100) or an `Object` for WebSockets/Bidirectional links (e.g., `{ forward: 80, backward: 20 }`). |
...
| `props` | `Object` | **Mandatory** | Map of property changes: `{ opacity, color, traffic, visible, style, details }`. |
